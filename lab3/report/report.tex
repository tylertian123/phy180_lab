...
\ref{sec:lab3b_correction} is plotted in Figure \ref{fig:lab3b_data_corrected}. Since the masses are spaced apart roughly
exponentially, the data is also plotted on a log scale.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Raw data for Period vs. Mass on a normal and log scale. The horizontal line at \(T = 2.079\) represents the
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Corrected data for Period vs. Mass on a normal and log scale according to Section \ref{sec:lab3b_correction}.
...
mass increases. While the data is inconclusive for determining the exact cause of this trend, there are two factors that
are likely causes:
\begin{enumerate}
    \item
...
With these factors potentially accounting for much of the apparent trend, it can be concluded that even if there was any
direct relationship between mass and period, its effect would be small enough to be negligible.
...
Nevertheless, their effects are relatively small, and with proper handling of uncertainties, most data points will be
in agreement, making the period effectively constant.

\textbf{Therefore, for most practical purposes, the simplification that mass does not \textit{directly} affect period is
a valid approximation for this pendulum.}
...
alignment of the measuring tape due to poor construction also introduces a large uncertainty. In future work, the
pendulum setup should be optimized and designed with minimizing uncertainties as the biggest priority, instead of
convenience and simplicity, and use more precise construction.
